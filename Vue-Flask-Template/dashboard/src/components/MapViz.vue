    ...
    methods: {
        ...
        async initChart() {
            ...
            if (this.selectedValue === "Mass Shooting") {
                ...
                let zoomStates = this.states;

                function transform2(d, states, zoomTransform) {
                ...
                function zoomed(e, d) {
                const { transform: zoomTransform } = e;
                g.attr("transform", `${zoomTransform} translate(65, 30)`);
                
                g.attr("stroke-width", 1 / zoomTransform.k);
                
                console.log('event: ', zoomTransform)
                console.log(zoomTransform.k)

                let ministates = d3.selectAll(".ministate")
                // .data(topojson.feature(data, data.objects.states).features.filter(d => {
                //     console.log('d: ', d)
                //     console.log('zoomStates: ', zoomStates)
                //     if (zoomStates.filter(s => s.state === d.properties.name) && zoomStates.filter(s => s.state === d.properties.name)[0]?.incidents_per_capita !== undefined) {
                //         console.log('treueueue')
                //         return true
                //     }
                //     else {
                //         return false;
                ...
                // }))
                .attr("transform", d => {
                    let oldTransform = transform(d, zoomStates);
                    console.log('transform: ', oldTransform)
                    console.log(zoomTransform)
                    return `${transform2(d, zoomStates, zoomTransform)}`
                });
                console.log('ministates: ', JSON.parse(JSON.stringify(ministates)))
                // .data(topojson.feature(data, data.objects.states).features.filter(d => {
                //     console.log('d: ', d)
                //     console.log('zoomStates: ', zoomStates)
                //     if (zoomStates.filter(s => s.state === d.properties.name) && zoomStates.filter(s => s.state === d.properties.name)[0]?.incidents_per_capita !== undefined) {
                ...
                //     }
                // }))
                // .attr("transform", d => transform2(d, zoomStates, 10))


                // state.attr("stroke-width", 1 / transform2(d, this.states, zoomTransform.k));
                    ...
                    .on('mousemove', (e, d) => {
                       let stateInfo = this.states.filter(s => s.state === d.properties.name)[0];
                        ...
                        d3.select('.d3-tooltip')
                            ...
                            console.log('stateInfo.state: ', stateInfo.state)

                            let circle = d3.selectAll(`.circlestate-${stateInfo.state}`)
                            .style('stroke', 'black')       // Change the stroke color to red
